...
# ============================================================
# GET /api/v1/export/report/<project_id>
# ============================================================
@export_bp.route("/report/<project_id>", methods=["GET"])
@login_required
def export_report(current_user, project_id):
    """导出选品报告 (PDF/Markdown)"""
    fmt = request.args.get("format", "pdf").lower()
    if fmt not in ("pdf", "md"):
        return jsonify({"success": False, "message": f"不支持的格式: {fmt}，支持: pdf/md"}), 400
    ...
    if db:
        try:
            ...
            report_data = {
                ...
                "products": [dict(r) for r in products] if products else [],
            }
        except Exception as e:
            logger.error(f"[Export] 查询报告数据失败: {e}")

    if not report_data:
        report_data = {
            "project": {"name": f"Project {project_id}", "keywords": "wireless earbuds", "marketplace": "US"},
            "products": _get_demo_products()[:5],
        }

    # Generate markdown report
    md_content = _generate_report_md(report_data)

    if fmt == "md":
        import io
        buf = io.BytesIO(md_content.encode("utf-8"))
        buf.seek(0)
        return send_file(buf, mimetype="text/markdown; charset=utf-8", as_attachment=True,
                         download_name=f"report_{project_id}.md")

    # PDF export
    try:
        exporter = DataExporter()
        buf = exporter.export_report_pdf(md_content, title=f"Sourcing Report - {report_data['project'].get('name', project_id)}")
        if buf is None:
        ...
        return send_file(buf, mimetype="application/pdf", as_attachment=True,
                         download_name=f"report_{project_id}.pdf")
    except Exception as e:
        logger.error(f"[Export] PDF 导出失败: {e}")
        return jsonify({"success": False, "message": f"PDF 导出失败: {str(e)}"}), 500
...
def _generate_report_md(data):
    ...
    lines = [
        f"# Sourcing Report: {p.get('name', 'Untitled')}",
        f"",
        ...
        f"",
        f"| # | ASIN | Title | Price | Rating | Reviews | BSR | Monthly Sales | Margin | Score |",
        f"|---|------|-------|-------|--------|---------|-----|---------------|--------|-------|",
    ]
    for i, prod in enumerate(products, 1):
        lines.append(
            f"| {i} | {prod.get('asin','-')} | {str(prod.get('title','-'))[:40]} | "
            f"${prod.get('price',0):.2f} | {prod.get('rating','-')} | {prod.get('review_count','-')} | "
            f"{prod.get('bsr_rank','-')} | {prod.get('monthly_sales','-')} | "
            f"{prod.get('profit_margin','-')}% | {prod.get('opportunity_score','-')} |"
        )
    lines.extend(["", "---", "", "*Generated by Amazon Visionary Sourcing Tool*"])
    return "\n".join(lines)
